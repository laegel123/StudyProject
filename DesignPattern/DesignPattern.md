...
### 7. 시스템을 디자인할 때 어떤 객체든 그 객체와 상호작용을 하는 클래스의 개수와 상호작용 방식에 주의를 기울여야 한다. (Principle of Least Knowledge)
...
# 5. Singleton Pattern (싱글턴 패턴)
   싱글턴 패턴은 클래스 인스턴스를 하나만 만들고, 그 인스턴스로의 전역 접근을 제공하는 것이다.

      클래스로더, 리플렉션, 직렬화, 역직렬화 사용 시 문제가 될 수 있다.
      이를 사용하는 고급 자바 이용자라면 이 점을 유의해야 한다.
      -> enum 으로 싱글턴을 생성해 해결할 수 있다.

   싱글턴 패턴을 사용하면 불가피하게 느슨한 결합 원칙을 위반하게 된다. 이 점은 문제점으로 종종 제기된다.  
   또한 싱글턴은 자신의 인스턴스를 관리하는 일 외에도 원래 그 인스턴스를 사용하고자 하는 목적에 부합하는 작업을 책임져야 한다.  
   따라서 '한 클래스가 한 가지만 책임지기' 원칙을 지키기 어렵다. 
...
# 7. Adapter Pattern (어댑터 패턴)
   어댑터 패턴은 특정 클래스 인터페이스를 클아이언트에서 요구하는 다른 인터페이스로 변환한다.  
   인터페이스가 호전되지 않아 같이 쓸 수 없었던 클래스를 사용할 수 있게 도와준다.
